...
func init() {
	...
	tunCmd.PersistentFlags().StringP("user-identity", "s", defaultIdentity, "the ssh identity (private) key absolute path")
	tunCmd.PersistentFlags().StringP("known-hosts", "k", knownHostFile, "the known_hosts file absolute path")
